...
class Graph:
    ...
    def addheuristicValue(self,value):
        if type(value) is dict:
            for node,h in value.items():
                if node in self.getVertices():
                    self.getVertex(node).setHeuristic(h)
                else:
                    print(f'{node} Vertex is not in the graph, please put a vertex that already exists in the graph')    
    ...
    def calculateCost(self):
        ...
        for count,track in enumerate(self.allPaths):
            ...
            for x,y in zip_longest(track,track[1:]):
                if y==None:
                    number += self.getVertex(x).getHeuristic()
                    break
    ...
    def AstartAlgorithm(self,frm,to):
        self.allPaths=[]
...
if __name__ == '__main__':
    ...
    heuristicValue={'a':2,'b':3,'s':7,'d':2,'g':4,'e':2,'f':4}
    G.addheuristicValue(heuristicValue)

    path=G.AstartAlgorithm('s','e') 
    ...
    ax = plt.gca()
    if path[0]=='No Path':
        ax.set_title(f'A star algorithm path from node {repr(path[1])} to node {repr(path[2])} :\n {str(path[0])}')
    else:
        ax.set_title(f'A star algorithm path from node {repr(path[0])} to node {repr(path[-1])} :\n {str(path)}')
